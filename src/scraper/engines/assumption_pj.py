...
class AssumptionPjEngine(BaseEngine):
	...
	def scrape(self):
		...
		try:
			...
			if not pdf_response:
				raise ValueError("No reachable bulletin PDF found for the current or previous week.")
		...
		try:
			...
			raw_text = self._generate_content_with_retry(client, vision_image)
			schedules = self._parse_model_output(raw_text)
			schedules = self._normalize_schedules(schedules)
			
			# UPDATE: Automatically inject Saturday Sunset Mass if missing
	...
	def _generate_content_with_retry(self, client, vision_image):
		...
		for model_name in models_to_try:
			...
			for attempt in range(max_retries):
				try:
					...
					raise e
		raise RuntimeError("All configured Gemini models failed due to high demand capacity limits.")
	...
	def _parse_model_output(self, raw_text):
		...
		if cleaned.startswith("```"):
			cleaned = re.sub(r"^http://googleusercontent.com/immersive_entry_chip/0json)?\s*", "", cleaned, flags=re.IGNORECASE)
