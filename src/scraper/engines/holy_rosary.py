...
class HolyRosaryEngine(BaseEngine):
    ...
    def _find_latest_bulletin_link(self, soup):
        ...
        if not candidates:
            raise ValueError(
                "No bulletin PDF row matching 'Bulletin DDMMYYYY.pdf' was found."
            )
    ...
    def _download_postback_pdf(self, session, headers, page_soup, event_target):
        if not event_target:
            raise ValueError(
                "Missing __doPostBack event target for bulletin download row."
            )
    ...
    def _parse_schedule_text(self, pdf_text):
        ...
        for section_language, section_text in sections:
            schedules.extend(
                self._parse_section(section_text, section_language, bulletin_season)
            )
    ...
    def _parse_section(self, section_text, default_language, bulletin_season=None):
        ...
        for index, day_match in enumerate(day_matches):
            ...
            season_notes = self._extract_notes(
                chunk, day_name, None, None, bulletin_season
            )
            ...
            if len(time_matches) == 1 or self._looks_like_time_range(time_matches):
                ...
                end_time = (
                    self._normalize_time(end_raw)
                    if end_raw
                    else self._plus_one_hour(start_time)
                )
    ...
    def _is_mass_section_block(self, text, block_x0, block_x1, page_midpoint):
        ...
        mass_heading_pattern = re.compile(
            r"Mass Times and Mass Intentions|Mass Times|Mass Intentions", re.IGNORECASE
        )
    ...
    def _extract_notes(self, chunk, day_name, start_raw, end_raw, bulletin_season=None):
        ...
        bulletin_season = (
            bulletin_season
            if bulletin_season is not None
            else getattr(self, "_bulletin_season_context", None)
        )
    ...
    def _infer_language_for_time_segment(
        self, chunk, time_matches, time_index, default_language
    ):
        segment_start = time_matches[time_index].start()
        segment_end = (
            time_matches[time_index + 1].start()
            if time_index + 1 < len(time_matches)
            else len(chunk)
        )
        segment = chunk[segment_start:segment_end]

        if re.search(r"\(\s*E\s*\)", segment, re.IGNORECASE) or re.search(
            r"\bE\b", segment, re.IGNORECASE
        ):
            return "English"

        if re.search(r"\(\s*M\s*\)", segment, re.IGNORECASE) or re.search(
            r"\bM\b", segment, re.IGNORECASE
        ):
            return "Mandarin"

        if len(time_matches) > 1 and re.search(
            r"\(\s*E\s*\).*\(\s*M\s*\)", chunk, re.IGNORECASE
        ):
    ...
    def _extract_liturgical_season(self, chunk):
        season_patterns = (
            (
                re.compile(
                    r"\b\d+(?:st|nd|rd|th)\s+Week\s+(?:in|of)\s+Ordinary\s+Time\b",
                    re.IGNORECASE,
                ),
                None,
            ),
            ...
            (
                re.compile(
                    r"\b\d+(?:st|nd|rd|th)\s+Week\s+of\s+Easter\b", re.IGNORECASE
                ),
                None,
            ),
        ...
        for pattern, canonical_label in season_patterns:
            ...
            if not match:
                continue
            return (
                match.group(0).strip() if canonical_label is None else canonical_label
            )
    ...
    def _extract_bulletin_season_context(self, pdf_text):
        lines = [
            line.strip()
            for line in (pdf_text or "").replace("\r", "\n").splitlines()
            if line.strip()
        ]
    ...
    def _extract_ordinary_time_week_number(self, season_text):
        ...
        match = re.search(
            r"\b(?P<week>\d+)(?:st|nd|rd|th)?\s+Week\s+(?:in|of)\s+Ordinary\s+Time\b",
            season_text,
            re.IGNORECASE,
        )
        if not match:
    ...
    def _extract_bulletin_date_range(self, pdf_text):
        ...
        lines = [
            line.strip()
            for line in pdf_text.replace("\r", "\n").splitlines()
            if line.strip()
        ]
        for line in lines[:12]:
            for pattern in date_patterns:
                ...
                day_text = re.sub(
                    r"(st|nd|rd|th)$", "", match.group("day"), flags=re.IGNORECASE
                )
                month_text = match.group("month")
                year_text = match.group("year")
                parsed = datetime.strptime(
                    f"{day_text} {month_text} {year_text}", "%d %B %Y"
                )
                return parsed.strftime("%Y-%m-%d"), (
                    parsed + timedelta(days=6)
                ).strftime("%Y-%m-%d")
